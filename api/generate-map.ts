...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!body.systemInstructions || typeof body.systemInstructions !== 'string') {
    return res.status(400).json({ detail: 'systemInstructions is required' });
  }

  try {
    const upstream = await fetch(`${gatewayBase()}/api/generate-map`, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        ...(typeof body.isPlannerMode === 'boolean' ? { isPlannerMode: body.isPlannerMode } : {}),
